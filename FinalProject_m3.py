# 2022년도 2학기 데이터마이닝 - FinalProject_m3
# 이름 : 이지윤, 정유라, 김송우
...
def set_result(clusters, score):
    with open(f'./finalproj_output_m3.txt',
              'w') as file:
...
def merging_by_initial_clique(edges, genes, graph, clusters) :
    ...
    # print(f'result : {len(sum(clusters, []))}')
    ...
    for gene in not_clustered_genes :
        ...
        # if len(neighbors) == 1 :
        #     cluster_idx_of_main_gene_ngbr = get_cluster_of_gene(gene, clusters)
        #     clusters[cluster_idx_of_main_gene_ngbr].add(gene)
        ...
        # if len(max_jcd_indice) == 1 :
        #     max_jcd_idx = max_jcd_indice[0]
        #     edges_sub[max_jcd_idx].remove(gene)
        #     neighbor_add = edges_sub

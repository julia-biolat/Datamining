# 2022년도 2학기 데이터마이닝 - FinalProject_m1
# 이름 : 이지윤, 정유라, 김송우
...
def set_result(clusters, score):
    with open(f'./finalproj_output_m1.txt',
              'w') as file:
...
def recur1(genes, gene, bfr_jcd_coef, entire_genes, coefs, clusters):
    ...
    if len(cluster) == 1:
        neighbors = [n for n in genes[gene] if (n not in entire_genes) and (n not in cluster[0])] # 이전에 포함된 것들 뺴고 추가해보자
        for entire_gene in entire_genes :
            cluster_temp = [cluster for cluster in clusters if entire_gene in cluster]
            if len(cluster_temp) == 0 :
                continue
            neighbors = [n for n in neighbors if n not in cluster_temp[0]]
    else:
        neighbors = [n for n in genes[gene] if (n not in entire_genes)]  # 이전에 포함된 것들 뺴고 추가해보자
    if len(neighbors) == 0 :
        # entire_genes.append(gene)
    ...
    for neighbor_jcd_max in neighbors_jcd_max:
        ...
        if val_max <= val :
            val_max = val
            entire_res = entire_new
    return entire_res, val_max


def merge_clusters(qwe, entire_res, clusters):
    cluster_indice = []
    ...
    a = len(cluster_indice)
    cluster_indice = list(set(cluster_indice))
    b = len(cluster_indice)
...
def merging_by_initial_clique(edges, genes, graph, clusters) :
    ...
    for gene in not_clustered_genes :
        ...
        clusters = merge_clusters(gene, entire_res, clusters)
    return clusters
